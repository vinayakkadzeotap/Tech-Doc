...
const QUIZZES: QuizData[] = [
  {
    ...
    questions: [
      ...
      { id: 'be11', text: 'What is "audience addressability" and why does it matter post-cookie?', options: ['The size of your email list', 'The ability to reach and identify specific audience segments across channels without relying on third-party cookies — Zeotap solves this through first-party identity resolution', 'A measure of ad spend efficiency', 'The number of ad platforms you\'re connected to'], correct: 1, explanation: 'Addressability is the ability to reach known individuals across channels. As third-party cookies die, addressability drops. Zeotap restores it by building first-party identity graphs that connect users across devices and channels without cookies.' },
      { id: 'be12', text: 'What role does data governance play in Zeotap\'s platform?', options: ['It\'s an optional add-on', 'It ensures data quality, consent compliance, access control, and audit trails are enforced throughout the entire data lifecycle — from collection to activation', 'It only applies to EU customers', 'Data governance is handled entirely by the customer'], correct: 1, explanation: 'Data governance in Zeotap covers: quality rules (reject bad data), consent enforcement (only use consented data), access controls (who can see what), and audit trails (what happened to data and when). It\'s built into the platform, not optional.' },
    ],
  },
  {
  ...
  },
  {
    ...
    questions: [
      ...
      { id: 'pm6', text: 'How do you set up a multi-step customer journey that sends an email, waits 3 days, then sends a push notification only to non-openers?', options: ['This isn\'t possible in Zeotap', 'Use Journey Canvas: add an email action node → wait node (3 days) → condition node (email opened = false) → push notification action node on the "no" branch', 'Write custom code in the API console', 'Create two separate campaigns manually'], correct: 1, explanation: 'Journey Canvas supports branching logic: action nodes (send email), wait nodes (time delays), and condition nodes (check if a previous action had a result). This enables sophisticated multi-step campaigns without code.' },
      { id: 'pm7', text: 'What is the correct way to debug why an audience segment has fewer members than expected?', options: ['Delete the segment and recreate it', 'Use Profile Explorer to check individual profiles against the segment rules, verify data source freshness on the Data Sources page, and check the segment rule logic for overly restrictive conditions', 'Contact engineering support immediately', 'Wait 24 hours and check again'], correct: 1, explanation: 'Debugging segment sizes requires checking multiple layers: Are profiles actually receiving the expected events/attributes? (Profile Explorer) Is data flowing? (Data Sources page) Are the rules too restrictive? (Segment rule review) Systematic debugging saves time.' },
      { id: 'pm8', text: 'What does "field mapping" in a destination setup actually control?', options: ['The physical location of servers', 'Which Zeotap profile attributes get sent to which fields in the destination platform — e.g., mapping "email_hash" in Zeotap to "hashed_email" in Google Ads', 'The order in which data is processed', 'Database table relationships'], correct: 1, explanation: 'Field mapping bridges the gap between Zeotap\'s data model and the destination\'s expected format. Without correct mapping, data arrives but can\'t be used — e.g., if Google Ads expects "hashed_email" but receives "email_sha256", the integration fails silently.' },
      { id: 'pm9', text: 'How can you verify that a destination sync is working correctly?', options: ['You can\'t — just trust it works', 'Check the Destinations page for sync status, review the last sync timestamp and record count, verify data appears in the destination platform, and monitor for error alerts', 'Only by logging into the destination platform directly', 'Send a test email to yourself'], correct: 1, explanation: 'Verify at multiple levels: Zeotap\'s destination page (sync status, timestamps, record counts), the destination platform itself (did audiences appear?), and monitoring (are there error alerts?). Never assume — always verify the full chain.' },
    ],
  },
  {
  ...
  },
  {
    ...
    questions: [
      ...
      { id: 'csk11', text: 'A customer reports that their audience sync to Google Ads has stopped updating. The integration shows "Connected" status. What should you check?', options: ['Tell them to contact Google directly', 'Check the sync schedule, verify the Google Ads OAuth token hasn\'t expired, review the audience size (Google requires minimum 1000 users), and check for rate limit errors in the sync logs', 'Disconnect and reconnect the entire integration', 'Wait for the next scheduled sync'], correct: 1, explanation: 'A "Connected" status doesn\'t mean syncs are working. Common issues: expired OAuth tokens (Google tokens expire), audience too small (Google requires 1000+ members), rate limits hit, or field mapping changes on Google\'s side. Always check the sync logs for specific errors.' },
      { id: 'csk12', text: 'What is the ideal cadence for customer touchpoints by account tier?', options: ['All customers get weekly calls', 'Tier 1 (strategic): weekly syncs + monthly executive check-ins. Tier 2 (growth): bi-weekly syncs. Tier 3 (standard): monthly syncs + quarterly reviews. All tiers get proactive health monitoring', 'Only contact customers when they open support tickets', 'Monthly calls for all customers regardless of size'], correct: 1, explanation: 'Touchpoint cadence should match account importance and complexity. Strategic accounts need frequent, deep engagement. Growth accounts need regular but efficient touchpoints. Standard accounts benefit from scaled programs with proactive monitoring.' },
    ],
  },
  {
    ...
    questions: [
      ...
      { id: 'tam9', text: 'What is the difference between a "warehouse-native" and "warehouse-connected" CDP deployment?', options: ['They are the same thing', 'Warehouse-native runs all processing inside the customer\'s warehouse (data never leaves); warehouse-connected syncs data between the CDP and warehouse bidirectionally, with processing in the CDP', 'Warehouse-native is always better', 'Warehouse-connected means no cloud is involved'], correct: 1, explanation: 'Warehouse-native (zero-copy) processes data in-place inside Snowflake/BigQuery — ideal for strict data governance. Warehouse-connected syncs data to the CDP for processing — ideal for customers wanting rich real-time capabilities. TAMs must assess which model fits each customer.' },
      { id: 'tam10', text: 'When conducting an architecture review for a new enterprise customer, what should you assess first?', options: ['What competitors they are evaluating', 'Their current data landscape: what sources exist, where data lives, how it flows between systems, what identity keys are available, and what their data quality baseline looks like', 'Their marketing team\'s campaign calendar', 'Their IT budget'], correct: 1, explanation: 'Architecture reviews start with understanding the current state: data sources (CRM, web, mobile, POS, etc.), data flows (batch vs. real-time), identity keys (email, phone, device IDs), and data quality. Without this baseline, you can\'t design the right integration architecture.' },
    ],
  },
];
