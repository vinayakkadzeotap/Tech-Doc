/* ═══════════════════════════════════════════════════════════════
   ZEOTAP CDP — KNOWLEDGE GRAPH ENGINE
...
(function(global) {
  ...
  /* ── GRAPH DATA ─────────────────────────────────────────── */
  var NODES = [
    { id:'collect',   label:'Data Collection',   category:'ingestion',   url:'02-data-collection.html',
    ...
    { id:'ingest',    label:'Data Ingestion',     category:'ingestion',   url:'03-data-ingestion.html',
    ...
    { id:'identity',  label:'Identity Graph',     category:'core',        url:'03-identity-resolution.html',
    ...
    { id:'profiles',  label:'Profile Store',      category:'storage',     url:'05-profile-store.html',
    ...
    { id:'audiences', label:'Audiences',          category:'activation',  url:'06-audience-management.html',
    ...
    { id:'journeys',  label:'Customer Journeys',  category:'activation',  url:'07-customer-journeys.html',
    ...
    { id:'activation',label:'Data Activation',    category:'activation',  url:'08-data-activation.html',
      ...
      tech:['Java','REST','gRPC'] },
    { id:'ai',        label:'AI / Zoe',           category:'intelligence',url:'04-genai-zoe.html',
    ...
    { id:'ml',        label:'ML Platform',        category:'intelligence',url:'10-ml-platform.html',
    ...
    { id:'reporting', label:'Reporting & BI',     category:'analytics',   url:'11-reporting-bi.html',
    ...
    { id:'unity',     label:'Unity Dashboard',    category:'analytics',   url:'12-unity-dashboard.html',
    ...
    { id:'privacy',   label:'Privacy & GDPR',     category:'compliance',  url:'13-privacy-gdpr.html',
    ...
    { id:'auth',      label:'Auth & IAM',         category:'security',    url:'14-auth-iam.html',
    ...
    { id:'infra',     label:'Infrastructure',     category:'platform',    url:'15-infrastructure.html',
    ...
    { id:'observe',   label:'Observability',      category:'platform',    url:'16-observability.html',
    ...
    { id:'cicd',      label:'CI/CD',              category:'platform',    url:'17-cicd.html',
    ...
    { id:'testing',   label:'Testing & QA',       category:'platform',    url:'18-testing.html',
  ...
  function init(containerId) {
    ...
    for (var x = 0; x < W; x += step) {
      grid.append('line')
        ...
        .attr('stroke', 'rgba(255,255,255,0.02)').attr('stroke-width', 1);
    }
    ...
    var links = LINKS.map(function(d) { return Object.assign({}, d); });

    /* Simulation */
    var sim = d3.forceSimulation(nodes)
      .force('link',    d3.forceLink(links).id(function(d) { return d.id; }).distance(145).strength(0.38))
      .force('charge',  d3.forceManyBody().strength(-680))
      .force('center',  d3.forceCenter(W / 2, H / 2))
      .force('collide', d3.forceCollide().radius(52).strength(0.85))
      .force('x',       d3.forceX(W / 2).strength(0.025))
      .force('y',       d3.forceY(H / 2).strength(0.025));

    /* Links */
    var linkG = svg.append('g').attr('class', 'kg-links');
    var link = linkG.selectAll('line').data(links).enter()
      .append('line')
      .attr('class', 'kg-link')
    ...
    var linkLabel = labelG.selectAll('text').data(links).enter()
      .append('text')
    ...
    var node = nodeG.selectAll('g').data(nodes).enter()
      .append('g').attr('class', 'kg-node')
      .call(d3.drag()
        .on('start', function(e, d) { if (!e.active) sim.alphaTarget(0.3).restart(); d.fx = d.x; d.fy = d.y; svg.style('cursor','grabbing'); })
        .on('drag',  function(e, d) { d.fx = e.x; d.fy = e.y; })
        .on('end',   function(e, d) { if (!e.active) sim.alphaTarget(0); d.fx = null; d.fy = null; svg.style('cursor','grab'); })
      );
    ...
    node.append('text')
      ...
      .attr('font-size', '10.5px').attr('font-weight', '500')
    ...
    node
      .on('mouseenter', function(event, d) {
        ...
        linkLabel.attr('fill', function(l) {
          var s = typeof l.source === 'object' ? l.source.id : l.source;
          var t = typeof l.target === 'object' ? l.target.id : l.target;
          return (s === d.id || t === d.id) ? 'rgba(148,163,184,0.75)' : 'rgba(107,136,170,0.05)';
        });

        d3.select(this).select('.kg-node-circle')
          .attr('r', 24).attr('stroke-width', 2)
          .attr('fill', function(n) { return CATEGORY[n.category].color + '35'; });
        ...
        if (tooltip) {
          ...
          tooltip.style.left = Math.min(ex + 18, W - 268) + 'px';
          tooltip.style.top  = Math.max(ey - 50, 8) + 'px';
      ...
      .on('mouseleave', function() {
        ...
        linkLabel.attr('fill', 'rgba(107,136,170,0.3)');
        d3.select(this).select('.kg-node-circle')
      ...
      .on('click', function(event, d) {
        event.preventDefault(); window.location.href = d.url;
      });
    ...
    global.KG = {
      ...
      reheat: function() { sim.alpha(0.4).restart(); }
    };
